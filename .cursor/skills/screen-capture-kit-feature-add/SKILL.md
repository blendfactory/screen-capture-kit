...
2. **Dart API:** `getShareableContent()` → `Window` list on `ShareableContent`; `createWindowFilter(Window)` → `FilterId`; `startCaptureStream(filter, frameSize: …)` / `startCaptureStreamWithUpdater(filter, frameSize: …)` → `Stream<CapturedFrame>` / `CaptureStream` (use `FrameSize.zero` or `FrameSize(width:, height:)`)
